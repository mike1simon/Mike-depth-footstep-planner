...
#include <depth_footstep_planner/FootstepPlannerEnvironment.h>
...
: DiscreteSpaceInformation(),
  ...
  ivMaxAttitudeForStep(params.MaxAttitudeForStep),
  ivCollisionCheckMethod(params.collision_check_method),
                                     ...
                                     const PlanningState& b)
{
  ...
  double attitude = abs(a.getDepth() - b.getDepth());
  double diffDepthCost = 0.0;
  if (ivHeuristicConstPtr->getHeuristicType() == Heuristic::EUCLIDEAN_STEPCOST)
  {
    boost::shared_ptr<EuclStepCostHeuristic> h =
        boost::dynamic_pointer_cast<EuclStepCostHeuristic>(
            ivHeuristicConstPtr);
    diffDepthCost = h->getDiffDepthCost();
  }
  else if(ivHeuristicConstPtr->getHeuristicType() == Heuristic::PATH_COST){
...
bool
FootstepPlannerEnvironment::occupied(State& s)
{
  PlanningState P(s, ivCellSize, ivNumAngleBins,ivHashTableSize);
  bool result = occupied(P);
  s.setDepth(P.getDepth());
  return result;
}
...
FootstepPlannerEnvironment::getState(unsigned int id, State* s)
{
  ...
  s->setDepth(planning_state->getDepth());
...
void
FootstepPlannerEnvironment::updateMap(depthmap2d::DepthMap2DPtr map)
{
  ivMapPtr.reset();
  ivMapPtr = map;
...
void FootstepPlannerEnvironment::updateModelOutput(const sensor_msgs::Image::ConstPtr& model_output)
{
  ...
  try
  {
    ...
    cv::Mat modelOutputGrid = cv::Mat(model_output->width, model_output->height,CV_8UC1);
    for(unsigned int j = 0; j < model_output->height; ++j){
      for(unsigned int i = 0; i < model_output->width; ++i){
        modelOutputGrid.at<uint8_t>(i, j) = temp_img.at<uint8_t>(j, i);
      }
    }
                                     ...
                                     std::vector<int> *CostV)
{
  ...
  //PlanningState* current = ivStateId2State[TargetStateID];
/// need to understand this section better
/// Cancel This Section for now
...
//  }
  ...
  for(footstep_set_iter = ivFootstepSet.begin();
      ...
      ++footstep_set_iter)
  {
    PlanningState predecessor =
        footstep_set_iter->reverseMeOnThisState(*current);
    bool footstep_Occupied = occupied(predecessor);
    bool footstep_AttitudeReachable = reachableFromDepth(*current,predecessor);
    if (footstep_Occupied || !footstep_AttitudeReachable)
      continue;
...
//  }
      ...
      ++footstep_set_iter)
  {
    ...
    bool footstep_Occupied = occupied(successor);
    bool footstep_AttitudeReachable = reachableFromDepth(*current,successor);
    if (footstep_Occupied || !footstep_AttitudeReachable)
      continue;
...
void FootstepPlannerEnvironment::GetRandomNeighs(const PlanningState* currentState, std::vector<int>* NeighIDV, std::vector<int>* CLowV, int nNumofNeighs, int nDist_c, bool bSuccs)
{
  ...
  for (int i = 0; i < nNumofNeighs && nAttempts < 5*nNumofNeighs; ++i, ++nAttempts)
  {
    ...
    bool footstep_Occupied = occupied(randomState);
    bool footstep_AttitudeReachable = reachableFromDepth(*currentState,randomState);
    if (!footstep_Occupied && footstep_AttitudeReachable)
    {
...
FootstepPlannerEnvironment::AreEquivalent(int StateID1, int StateID2)
{
  ...
  return (std::abs(s1->getX() - s2->getX()) < 1
      ...
      //&& std::abs(s1->getDepth() - s2->getDepth()) < 10*FLOAT_CMP_THR
      && s1->getLeg() == s2->getLeg()
  );
                                       ...
                                       FILE *fOut)
{
  ...
  if(bVerbose)
  {
    SBPL_FPRINTF(fOut, "X=%i Y=%i THETA=%i DEPTH=%f FOOT=%i\n",
                 s->getX(), s->getY(), s->getTheta(),s->getDepth() , s->getLeg());
  }
  else
  {
    SBPL_FPRINTF(fOut, "%i %i %i %f %i\n",
                 s->getX(), s->getY(), s->getTheta(),s->getDepth() , s->getLeg());
  }
}
    ...
    for (int step_x = ivMaxInvFootstepX; step_x <= ivMaxFootstepX; ++step_x)
    {
           ...
           ++step_theta)
      {
        ...
        if (ivForwardSearch)
        {
          ...
          bool footstep_Occupied = occupied(pred);
          bool footstep_Reachable = reachable(left,pred);
          if (footstep_Occupied || !footstep_Reachable)
            continue;
...
//            continue;
          ...
          footstep_Occupied = occupied(pred);
          footstep_Reachable = reachable(right,pred);
          if (footstep_Occupied || !footstep_Reachable)
            continue;
...
//            continue;
        ...
        else
        {
          ...
          bool footstep_Occupied = occupied(succ);
          bool footstep_Reachable = reachable(left,succ);
          if (footstep_Occupied || !footstep_Reachable)
            continue;
...
//            continue;
          ...
          footstep_Occupied = occupied(succ);
          footstep_Reachable = reachable(right,succ);
          if (footstep_Occupied || !footstep_Reachable)
            continue;
