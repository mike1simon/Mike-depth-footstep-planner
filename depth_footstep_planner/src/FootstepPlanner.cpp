...
using depthmap2d::DepthMap2D;
using depthmap2d::DepthMap2DPtr;
  ...
  ivMarkerNamespace("")
{
  ...
  double diff_angle_cost, diff_depth_cost , dist_cost;
...
FootstepPlanner::run()
{
  ...
  else
  {
    ...
    ROS_ERROR("number of expanded state : %d",ivPlannerEnvironmentPtr->getNumExpandedStates());
...
// should change to footstep with x y z theta
void
...
FootstepPlanner::mapCallback(
    const depthmap_humanoid_msgs::DepthMapConstPtr& depth_map)
{
  depthmap2d::DepthMap2DPtr map(new depthmap2d::DepthMap2D(depth_map));
...
bool
FootstepPlanner::setGoal(State left_foot, State right_foot)
{
...
bool
FootstepPlanner::updateMap(const depthmap2d::DepthMap2DPtr map)
{
  // store old map pointer locally
  depthmap2d::DepthMap2DPtr old_map = ivMapPtr;
  // store new map
  ivMapPtr.reset();
  ivMapPtr = map;
...
void
FootstepPlanner::updateEnvironment(const depthmap2d::DepthMap2DPtr old_map)
{
...
FootstepPlanner::broadcastPathVis()
{
  ...
  for(path_iter = getPathBegin(); path_iter != getPathEnd(); ++path_iter)
  {
    ...
    state.pose.position.z = path_iter->getDepth();
                                  ...
                                  visualization_msgs::Marker* marker)
{
  ...
  marker->pose.position.z = foot_pose.getDepth() + ivEnvironmentParams.footsize_z*2;
